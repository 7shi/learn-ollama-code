...
# todo uses a complex nested schema (enum) that can't be expressed with type hints alone
TODO_TOOL = {"type": "function", "function": {"name": "todo", "description": "Update task list. Track progress on multi-step tasks.",
    "parameters": {"type": "object", "properties": {"items": {"type": "array", "items": {"type": "object",
    "properties": {"id": {"type": "string"}, "text": {"type": "string"}, "status": {"type": "string",
    "enum": ["pending", "in_progress", "completed"]}}, "required": ["id", "text", "status"]}}}, "required": ["items"]}}}
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        # Nag reminder is injected as a user message when the model hasn't used todo recently
        if rounds_since_todo >= 3:
